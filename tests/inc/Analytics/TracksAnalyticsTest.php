...
class TracksAnalyticsTest extends TestCase {
	...
	public function testInitDoesSetTracksIfTrackingIsEnabledViaFilter(): void {
		...
		$mock = Mockery::mock( TracksAnalytics::class )->makePartial();
		$mock->shouldReceive( 'is_enabled_via_filter' )->once()->with()->andReturn( true );
		$mock->shouldReceive( 'get_tracks_lib_class' )->once()->with()->andReturn( MockTracks::class );
		$mock->shouldReceive( 'setup_tracking_via_hooks' )->once()->with();
		
		TracksAnalytics::init();
		
		$this->assertEquals( true, TracksAnalytics::is_enabled_via_filter() );
		$this->assertEquals( MockTracks::class, $mock::get_tracks_lib_class() );
