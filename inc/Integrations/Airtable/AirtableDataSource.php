...
class AirtableDataSource extends HttpDataSource {
	...
	public function ___temp_get_query(): HttpQuery|WP_Error {
		...
		foreach ( $this->config['service_config']['tables'][0]['output_query_mappings'] as $mapping ) {
			...
			// Special handling for currency fields, which are implemented as a string
			// with a `format` function to get their formatted value.
			if ( 'currency' === $mapping['type'] && isset( $mapping['prefix'] ) ) {
				$output_schema['type'][ $mapping_key ]['format'] = [ FieldFormatter::class, 'format_currency' ];
				$output_schema['type'][ $mapping_key ]['type'] = 'string';
			}
		}
