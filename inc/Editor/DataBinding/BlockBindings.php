...
class BlockBindings {
	...
	public static function get_value( array $source_args, WP_Block|array $block, string $attribute_name ): ?string {
		...
		if ( ! empty( $field_label ) && 'content' === $attribute_name ) {
			return sprintf(
				'<span class="rdb-block-label">%s</span> %s',
				Sanitizer::sanitize_primitive_type( 'string', $source_args['label'] ),
				Sanitizer::sanitize_primitive_type( 'string', $value )
			);
		}

		return Sanitizer::sanitize_primitive_type( 'string', $value );
	}
