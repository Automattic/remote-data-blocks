...
class BlockBindings {
	...
	public static function get_value( array $source_args, WP_Block|array $block ): ?string {
		...
		$field_result = $query_response['results'][ $index ]['result'][ $field_name ] ?? null;
		$field_value = $field_result['value'] ?? null;

		if ( null === $field_result || null === $field_value ) {
			self::log_error( 'Cannot resolve field for block binding', $block_name, $field_name );
			return $fallback_content;
		}

		// Sanitize the value based on the field type.
		$value_type = $field_result['type'] ?? 'string';
		$value = Sanitizer::sanitize_primitive_type( $value_type, $field_value );
