...
class ConfigRegistry
{
	...
	// This is where the list of display query keys are stored.
	...
	public static function register_block(array $block_config = []): bool|WP_Error
	{
		...
		$configs_by_display_query = [];

		foreach ($block_config[self::DISPLAY_QUERIES_KEY] as $display_query_key) {
			if (isset($configs_by_display_query[$display_query_key])) {
				continue;
			}
			...
			$configs_by_display_query[$display_query_key] = [
				'queries' => $queries,
				'selectors' => $selectors,
			];

			$query = self::inflate_query($block_config[self::QUERIES_KEY][$display_query_key]['query']);
			$queries[$display_query_key] = $query;
			...
			// 	$required_query = self::inflate_query($block_config[self::QUERIES_KEY][$query->get_required_query()]['query']);
			...
			// $selectors[] = [
			// 	'display_name' => self::get_query_name_from_key($display_query_key),
			// 	'image_url' => $query->get_image_url(),
			// 	'inputs' => self::map_input_variables($input_schema),
			// 	'name' => self::DISPLAY_QUERY_KEY === $query->get_type() ? ($has_required_variables ? 'Manual input' : ($is_collection ? 'Load collection' : 'Load item')) : ucfirst($display_query_key),
			// 	'query_key' => $display_query_key,
			...
			$config = [
				...
				'selectors' => $selectors,
				'title' => $block_title,
				'inputs' => $block_config['queries'][ $display_query_key ]['inputs'] ?? [],
			];
			...
			$configs_by_display_query[$display_query_key] = $config;
		}

		ConfigStore::set_block_configuration($block_name, ['title' => $block_title, 'block_name' => $block_name, 'variations' => $configs_by_display_query]);
	...
	public static function get_query_by_key(array $queries, string $key): ?QueryInterface
	{
