...
class BlockRegistration
{
	...
	public static function register_container_blocks(): void
	{
		...
		foreach (ConfigStore::get_block_configurations() as $block_configuration) {
			$block_name = $block_configuration['block_name'];
	...
	public static function register_block_configuration(array $config): array
	{
		...
		foreach ($config['variations'] as $display_query_key => $variation) {
			...
			$display_query = ConfigRegistry::get_query_by_key( $variation['queries'], $display_query_key );
			...
			$block_configs['variations'][$display_query_key] = $block_config;
		}

		return [$block_configs, $script_handle];
	}
}
